<?php defined('BASEPATH') or exit('No direct script access allowed');
	class ppmod extends CI_Model
	{
		private $_table = 'tbl_pp';
		...
		public function simpanpp(){
			...
			$this-> no_pp    	= $post['txtnopp'];
			$this-> tgl_lapor	= date("Y-m-d");
			$this-> jam_lapor	= date("H:i:s");
			$this-> section		= $post["txtsection"];
			$this-> pelapor 	= $post["txtpelapor"];
			$this->	kerusakan 	= $post["txtpermasalahan"];
			$this-> tertuju		= $post['radio'];
			$this-> an			= $post['an'];
			$this-> status_pp	= "waiting";
			//-------------------------------------------------- cari no antrian
		...
		public function getpptahunan($sect){
		...
		public function getpptake($sect){
			$this->db->select('no_pp, tgl_lapor, kerusakan, status_pp, tertuju');
			$this->db->from('tbl_pp');
			$this->db->where("tertuju",$sect);
			$this->db->where("status_pp","waiting");
			//$this->db->where("month(tgl_lapor)",date('m'));
			//$this->db->where("year(tgl_lapor)",date('Y'));
			$rows=$this->db->get();
			return $rows->result();
		...
		public function getdetailpp($nopp){
